...
RUN set -eux; \
  if [ -f /etc/apt/sources.list ]; then \
    if [ "${APT_MIRROR}" != "http://deb.debian.org/debian" ]; then \
      sed -i "s|http://deb.debian.org/debian|${APT_MIRROR}|g" /etc/apt/sources.list; \
    fi; \
    if [ "${APT_SECURITY_MIRROR}" != "http://deb.debian.org/debian-security" ]; then \
  ...
  if [ "${INSTALL_FONTS}" = "1" ]; then \
    apt-get -o Acquire::ForceIPv4=true -o Acquire::Retries=5 -o Acquire::http::Timeout=30 -o Acquire::https::Timeout=30 install -y --no-install-recommends fonts-noto-cjk; \
  fi; \
